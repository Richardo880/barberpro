...
export async function POST(request: NextRequest) {
  try {
    ...
    // Solo ADMIN puede subir fotos de staff
      ...
      return NextResponse.json(
        {
          ...
          hint: uploadError.message.includes("not found")
            ? "El bucket 'staff' no existe. Créalo en Supabase Dashboard > Storage."
            : uploadError.message.includes("security") || uploadError.message.includes("policy")
            ? "Falta configurar políticas RLS en el bucket. Ve a Supabase Dashboard > Storage > staff > Policies."
            : undefined
        },
        { status: 500 }
      );
    ...
    // Obtener URL pública
    const publicUrl = getPublicUrl(STORAGE_BUCKETS.STAFF, uniqueFileName);
